...
export default async function CohortPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ buscar?: string; estado?: string }> }) {
  ...
  return (
    <AppShell userName={session.user.name}><div className="app-content"><BackLink href="/staff">Volver al resumen</BackLink><PageIntro description={`${cohort.startsAt.toLocaleDateString("es-MX")} – ${cohort.endsAt.toLocaleDateString("es-MX")} · ${cohort.timezone}`} eyebrow="Cohorte facilitada" title={cohort.name} />
      ...
      <section className="panel roster"><div className="panel__header"><div><p className="eyebrow">Seguimiento</p><h2>Estudiantes</h2></div><People /></div><form className="filter-bar" method="get"><label><span className="sr-only">Buscar estudiante</span><input defaultValue={query.buscar} name="buscar" placeholder="Buscar por nombre o correo…" type="search" /></label><select aria-label="Filtrar por estado" defaultValue={query.estado ?? "all"} name="estado"><option value="all">Todos los estados</option><option value="ok">Sin señales de atención</option><option value="attention">Requiere atención</option></select><button className="button button--ghost" type="submit">Filtrar</button></form>
        <div className="roster-table"><div className="roster-table__head"><span>Estudiante</span><span>Avance</span><span>Progreso</span><span>Última actividad</span><span>Estado</span><span>Mentoría</span><span /></div>{roster.rows.map((student) => { const percent = student.totalRequiredItems ? Math.round((student.completedRequiredItems / student.totalRequiredItems) * 100) : 0; const needsAttention = student.attentionReasons.length > 0; return <div className="roster-table__row" key={student.userId}><span className="person-cell"><Avatar color={needsAttention ? "clay" : "green"} name={student.displayName} size="sm" /><span><strong>{student.displayName}</strong><small>{student.email}</small></span></span><span>{student.completedRequiredItems} de {student.totalRequiredItems}</span><ProgressBar label={`${percent}%`} value={percent} /><span>{student.lastActivityAt ? student.lastActivityAt.toLocaleDateString("es-MX") : "Sin actividad"}</span><StatusPill tone={needsAttention ? "warm" : "good"}>{needsAttention ? <Warning /> : student.activityStatus === "active" ? <Check /> : <Clock />}{statusLabel(student.paceStatus)} · {student.submissionStatus} · {student.evaluationStatus}</StatusPill><span><strong>{student.directMentors.length ? student.directMentors.map((mentor) => mentor.name).join(", ") : "Sin mentor"}</strong><form action={assignCohortMentor} className="form-stack"><input name="studentUserId" type="hidden" value={student.userId} /><select aria-label={`Mentor para ${student.displayName}`} defaultValue=""><option disabled value="">Asignar o cambiar</option>{candidates.mentors.map((mentor) => <option key={mentor.id} value={mentor.id}>{mentor.name}</option>)}</select><button className="button button--ghost" type="submit">Guardar</button></form></span><Link aria-label={`Ver seguimiento de ${student.displayName}`} href={`/staff/cohortes/${cohort.id}/estudiantes/${student.userId}`}><ArrowRight /></Link></div>; })}</div>
      ...
      <section className="staff-grid">
        ...
        <article className="panel settings-form"><h2>Mentoría directa</h2><p>La mentoría personal se gestiona por estudiante y también cubre a quienes avanzan de forma autodidacta.</p><Link className="button button--ghost" href="/staff/mentoria">Gestionar mentoría directa <ArrowRight /></Link></article>
