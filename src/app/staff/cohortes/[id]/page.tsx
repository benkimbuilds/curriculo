...
export default async function CohortPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ buscar?: string; estado?: string }> }) {
  ...
  return (
    <AppShell userName={session.user.name}><div className="app-content"><Link className="back-link" href="/staff"><ArrowLeft /> Volver al resumen</Link><PageIntro description={`${cohort.startsAt.toLocaleDateString("es-MX")} – ${cohort.endsAt.toLocaleDateString("es-MX")} · ${cohort.timezone}`} eyebrow="Cohorte facilitada" title={cohort.name} />
