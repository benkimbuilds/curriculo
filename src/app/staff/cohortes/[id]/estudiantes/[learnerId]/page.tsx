...
import { Clock, Message, User } from "@/components/icons";
...
export default async function StudentTimelinePage({ params, searchParams }: { params: Promise<{ id: string; learnerId: string }>; searchParams: Promise<{ nota?: string; evaluacion?: string }> }) {
  ...
  return (
    <AppShell userName={session.user.name}><div className="app-content app-content--narrow"><BackLink href={`/staff/cohortes/${id}`}>Volver a {timeline.cohort.name}</BackLink><PageIntro description={timeline.learner.email} eyebrow="Seguimiento individual" title={timeline.learner.name} />
