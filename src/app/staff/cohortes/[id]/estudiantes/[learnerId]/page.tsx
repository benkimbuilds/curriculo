...
export default async function StudentTimelinePage({ params, searchParams }: { params: Promise<{ id: string; learnerId: string }>; searchParams: Promise<{ nota?: string; evaluacion?: string }> }) {
  ...
  return (
    <AppShell userName={session.user.name}><div className="app-content app-content--narrow"><Link className="back-link" href={`/staff/cohortes/${id}`}><ArrowLeft /> Volver a {timeline.cohort.name}</Link><PageIntro description={timeline.learner.email} eyebrow="Seguimiento individual" title={timeline.learner.name} />
