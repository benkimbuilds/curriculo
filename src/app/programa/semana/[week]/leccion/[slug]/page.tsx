...
export default async function LessonPage({ params, searchParams }: {
  ...
  return <AppShell userName={context.user.name}><div className="lesson-page">
    ...
    <div className="lesson-layout"><article className="lesson-article">
      ...
      {locale !== "en" ? <footer className={`lesson-complete${isLessonComplete(progress?.state) ? " is-complete" : ""}`}><div><span>{isLessonComplete(progress?.state) ? <><Check /> Avance guardado</> : "¿Terminaste las actividades?"}</span><strong>{isLessonComplete(progress?.state) ? "Este material está completado." : "Guarda tu avance."}</strong></div><LessonCompleteForm alreadyComplete={isLessonComplete(progress?.state)} contentVersion={week.contentVersion} enrollmentId={context.enrollment.id} lessonId={lesson.id} week={weekNumber} /></footer> : null}
