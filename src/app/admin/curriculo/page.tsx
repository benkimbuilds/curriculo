...
export default async function CurriculumAdminPage({ searchParams }: { searchParams: Promise<{ idioma?: string }> }) {
  ...
  const languageGate = <form className="admin-language-gate" method="get"><Warning /><div><strong>Fuente de auditoría en inglés</strong><p>Esta comparación está limitada a roles de currículo y desarrollo.</p></div><label className="switch"><input defaultChecked={showEnglish} name="idioma" type="checkbox" value="en" /><span /> Mostrar inglés</label><button className="button button--ghost" type="submit">Aplicar</button></form>;
  return (
    <AppShell userName={session.user.name}><div className="app-content"><PageIntro action={languageGate} description="Estado editorial y procedencia del currículo versionado." eyebrow="Administración de currículo" title="Control editorial" />
