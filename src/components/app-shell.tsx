    ...
    <div className="app-frame">
      ...
      <aside className="sidebar">
        ...
        <nav aria-label={role === "student" ? "Área de estudiante" : "Área de equipo"} className="sidebar__nav">
          {links.map(({ href, label, icon: Icon }) => <Link href={href} key={href}><Icon /> <span>{label}</span></Link>)}
        </nav>
      ...
      <header className="mobile-app-header"><Logo /><details><summary aria-label="Abrir navegación">Menú</summary><nav>{links.map(({ href, label }) => <Link href={href} key={href}>{label}</Link>)}<AccountMenu showProfile={role === "student"} /></nav></details></header>
